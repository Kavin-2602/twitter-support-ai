...
1. **Unreliable Evaluation Harness (Judge Hallucination + Instability):** Our LLM-as-a-judge automated evaluation cannot be trusted as a reliable proxy for human judgment. It shows both instability across runs and concrete evidence of hallucinated justifications when scoring drafted replies. This is a genuine limitation of the evaluation harness, not a calibration nuance, and any automated quality metrics in this project should be read with that caveat.
   - **Instability:** Human-judge agreement varies between 47% and 83% across two runs using word-for-word identical prompts, identical input data, and `temperature=0.0`. The 36-point spread is not noise — it is structural unreliability.
   - **Hallucinated justifications:** Spot-checking reveals the judge generating factually wrong reasoning. On `amz_thread_0204`, the judge scored routing=1 with the justification "no response drafted" — on a row with a 150-character reply. On `amz_thread_0174`, the judge scored routing=1 on an ESCALATE action for a Damaged/Wrong Item intent, stating "escalation is not the prescribed action" — directly contradicting the rubric it was given, which explicitly requires escalation for Damaged/Wrong Item. On `amz_thread_0277`, the judge scored groundedness=1 on a plain generic apology, redefining "grounding" to mean "substantive content" rather than "absence of hallucinated specifics." See `judge_agreement_report.md` for the full diagnostic.
   - **Full-dataset average routing score: 2.21/5 (199 rows).** Despite per-row unreliability, the aggregate directional signal is plausible corroboration of the failure analysis finding that ~63% of errors stem from escalation-rule design — but it cannot be read as a precise measurement.
2. **Annotator Drift:** The golden set was hand-labelled across two sessions after a mid-project data-loss incident where 132 rows had to be relabelled. This likely introduced annotator drift, meaning my own judgment criteria may have shifted between the first and second pass, skewing the "ground truth."
3. **Under-specified "Other" Bucket:** The "Other" intent category acts as a massive catch-all bucket. Its semantic boundaries are extremely fuzzy, which distorts the classification accuracy for adjacent categories by artificially penalizing the model when it correctly identifies a nuanced intent that I lazily grouped into "Other".
